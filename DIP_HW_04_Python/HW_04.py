...
def fftAndIfftImg():
    ...
    show_text.insert('insert', "The OpenCV FFT and IFFT algorithm spend " + str(timeSpend) + 
...
def idealLowPass():
    ...
    plt.title('Ideal low-pass spectrum (R = 60)')

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(idealLowPass_Img, cmap ='gray')
    plt.title('Image after ideal low-pass (R = 60)')
...
def idealHighPass():
    ...
    plt.title('Ideal high-pass spectrum (R = 60)')

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(idealHighPass_Img, cmap ='gray')
    plt.title('Image after ideal high-pass (R = 60)')
...
def gaussianHighPass():
    ...
    plt.title('Gaussian high-pass spectrum (D0 = 60)')

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(gaussianHighPass_Img, cmap ='gray')
    plt.title('Image after gaussian high-pass (D0 = 60)')
...
def gaussianLowPass():
    ...
    plt.title('Gaussian low-pass spectrum (D0 = ' + str(D0) + ")")

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(gaussianLowPass_Img, cmap ='gray')
    plt.title('Image after gaussian low-pass (D0 = '+ str(D0) + ")")
...
def butterworthHighPass():
    ...
    plt.title('Butterworth high-pass spectrum (D0 = 60, n = 5)')

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(butterworthHighPass_Img, cmap ='gray')
    plt.title('Image after butterworth high-pass (D0 = 60, n = 5)')
...
def butterworthLowPass():
    ...
    plt.title('Butterworth low-pass spectrum (D0 = 60, n = 5)')

    ifftImage = plt.subplot(1,2,2) 
    ifftImage.imshow(butterworthLowPass_Img, cmap ='gray')
    plt.title('Image after butterworth Low-pass (D0 = 60, n = 5)')
...
button_fftIfftImg = tk.Button(window, text = "FFT  and   IFFT", command = fftAndIfftImg)
...
gammaL_ = tk.Entry(gammaL_frame, width = 9)
gammaL_.pack()
...
D0_ = tk.Entry(D0_frame, width = 9)
D0_.pack()

# Homomorphic Filtering 
button_homomorphic = tk.Button(window, text = "Homomorphic   Filter", command = homomorphic)
button_homomorphic.place(x = 10, y = 375)

# Blur and Wiener
button_blurWiner = tk.Button(window, text = "Blur + Restoration", command = blurWiener)
button_blurWiner.place(x = 10, y = 435)

# Blur, Noise and Wiener
button_blurNoiseWiener = tk.Button(window, text = "Blur & Noise + Restoration", command = blurNoiseWiener)
button_blurNoiseWiener.place(x = 10, y = 465)

# Show Text
show_text = tk.Text(window, height = 37, width = 119)
